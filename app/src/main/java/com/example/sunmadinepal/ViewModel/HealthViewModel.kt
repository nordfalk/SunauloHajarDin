...
class HealthViewModel : ViewModel() {
    ...
    internal fun fetchEvent_06_09_Months(
        ItemName: String,
        ItemDescription: String,
        ItemName1: String,
        ItemDescription1: String,
        ...
        firestore.collection("Health").get().addOnCompleteListener {
            ...
            if (it.isSuccessful) {
                for (document in it.result!!) {
                    ...
                    val ItemImage = document.data.getValue("app_go_to_healthpost").toString()
                    val ItemImage1 =
                        document.data.getValue("feeding6To9ImageUrl").toString()
//                    val ItemImage2 =document.data.getValue("app_boiledwater").toString()
    ...
    internal fun fetchEvent_09_12_Months(
        ItemName: String,
        ItemDescription: String,
        ItemName1: String,
        ItemDescription1: String,
        ...
        firestore.collection("Health").get().addOnCompleteListener {
            ...
            if (it.isSuccessful) {
                for (document in it.result!!) {
                    ...
                    _events.value =
                        listOf(RecipesData(ItemImage, itemName, itemDescription.replace("_b", "\n"), "")).plus(
                            listOf(RecipesData(ItemImage1, itemName1, itemDescription1, ""))
                        ).plus(listOf(RecipesData(ItemImage2, itemName2, itemDescription2, "")))
                }
            }
    ...
    internal fun fetchEvent_09_12_MonthsHealt(ItemName: String, ItemDescription: String) {
        firestore.collection("Health").get().addOnCompleteListener {
            if (it.isCanceled) {
                Log.e("Error", " Error in database")
            }
            if (it.isSuccessful) {
                for (document in it.result!!) {
                    ...
                    val ItemImage = document.data.getValue("9To12MonthFeedingImageUrl").toString()
        ...
        firestore.collection("Health").get().addOnCompleteListener {
            ...
            if (it.isSuccessful) {
                for (document in it.result!!) {
                    ...
                    val ItemImage = document.data.getValue("12To24FeedingImageUrl").toString()
    ...
    internal fun fetchEvent_GeneralHealth(
        ItemName: String, ItemDescription: String,
        ItemName1: String, ItemDescription1: String,
        healthyEatingTitle: String, healthyEatingDescriptoin: String,
        ItemName2: String, ItemDescription2: String,
        ItemName3: String, ItemDescription3: String,
        ItemName4: String, ItemDescription4: String,
        ItemName5: String, ItemDescription5: String,
        ItemName7: String, ItemDescription7: String,
        ItemName8: String, ItemDescription8: String
    ) {
        firestore.collection("GeneralHealth").get().addOnCompleteListener {
            ...
            if (it.isSuccessful) {
                for (document in it.result!!) {
                    ...
                    val ItemImage2 = document.data.getValue("malNutritionImageUrl").toString()
                    val vitaminAImageUrl= document.data.getValue("vitaminAImageUrl").toString()
                    val ironElementImage = document.data.getValue("ironElementImage").toString()
                    val ItemImage3 = document.data.getValue("kopi_af_vegetable").toString()
                    val caringForSickImageUrl = document.data.getValue("caringForSickImageUrl").toString()
                    val TDVaccineImageUrl = document.data.getValue("TDVaccineImageUrl").toString()
                    val healthyEatingImage = document.data.getValue("healthyEathingHabitsImage").toString()
...
//                    val itemDescription6= document.data.getValue(ItemDescription6).toString()
                    ...
                    val healthyEatingTitle = document.data.getValue(healthyEatingTitle).toString()
                    val healthyEatingDescription = document.data.getValue(healthyEatingDescriptoin).toString()
                    _events.value =
                        listOf(RecipesData(ItemImage, itemName, itemDescription, "")).plus(
                            listOf(RecipesData(ItemImage1, itemName1, itemDescription1, ""))
                        ).plus(listOf(RecipesData(healthyEatingImage, healthyEatingTitle, healthyEatingDescription, "")))
                            .plus(listOf(RecipesData(ItemImage2, itemName2, itemDescription2, "")))
                            .plus(listOf(RecipesData(vitaminAImageUrl, itemName3, itemDescription3, "")))
                            .plus(listOf(RecipesData(ironElementImage, itemName4, itemDescription4, "")))
...
//                    plus(listOf(RecipesData(ItemImage3 ,itemName6, itemDescription6 ,""))).
                        plus(listOf(RecipesData(TDVaccineImageUrl, itemName7, itemDescription7, "")))
                            .plus(listOf(RecipesData(caringForSickImageUrl, itemName8, itemDescription8, "")))
                }
            }
        }.addOnFailureListener { exception ->
            Log.d(ContentValues.TAG, "get failed with ", exception)
    ...
    internal fun fetchEvent_Maternity(
        ItemName: String,
        ItemDescription: String,
        ItemName1: String,
        ItemDescription1: String,
        ...
        firestore.collection("Health").get().addOnCompleteListener {
            ...
            if (it.isSuccessful) {
                for (document in it.result!!) {
                    val ItemImage = document.data.getValue("pregnancyExaminatinImageUrl").toString()
                    val ItemImage1 = document.data.getValue("app_balanceddiet_withmeat").toString()
                    val ironTabletImageUrl = document.data.getValue("ironTabeltImageUrl").toString()
                    ...
                    _events.value =
                        listOf(RecipesData(ItemImage, itemName, itemDescription, "")).plus(
                            listOf(RecipesData(ItemImage1, itemName1, itemDescription1, ""))
                        ).plus(listOf(RecipesData(ironTabletImageUrl    , itemName2, itemDescription2, "")))
